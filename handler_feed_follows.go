...
import (
	...
	"github.com/AmandaKhol/rss/internal/database"
...
func (apiCfg *apiConfig) handlerGetFollows(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	feedsFollows, err := apiCfg.DB.GetFeedFollow(r.Context(), user.ID)
	if err != nil {
		respondWithError(w, 400, fmt.Sprintf("Could not get feed follows: %s", err))
		return
	}

	respondWithJSON(w, 200, databaseFeedFollowsToFeedsFollows(feedsFollows))
}
...
func (apiCfg *apiConfig) handlerDeleteFeedFollow(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		respondWithError(w, 400, fmt.Sprintf("Could not delete feed follow id: %s", err))
		return
	}
